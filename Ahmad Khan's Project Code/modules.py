...
class TemporalConvBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert kernel_width % 2 == 1, "kernel_width should be odd"
        pad = kernel_width // 2

        self.conv = nn.Sequential(
            nn.Conv1d(channels, channels, kernel_size=kernel_width, padding=pad),
...
class CNNBiGRUEncoder(nn.Module):
    def __init__(
        ...
        self.input_proj = (
            nn.Identity()
            if conv_channels == num_features
            else nn.Linear(num_features, conv_channels)
        )
