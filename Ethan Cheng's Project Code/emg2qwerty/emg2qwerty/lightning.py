...
class TDSConvLSTMCTCModule(pl.LightningModule):
    """Hybrid CNN+LSTM: TDSConv extracts local features, LSTM models sequences."""
    ...
    def __init__(
        ...
        # LSTM encoder: temporal sequence modeling on top of CNN features
        self.lstm = nn.LSTM(
            input_size=num_features,
            hidden_size=hidden_size,
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        x = self.cnn_encoder(x)         # (T', N, num_features) — CNN shrinks T
        x, _ = self.lstm(x)             # (T', N, lstm_output_size)
